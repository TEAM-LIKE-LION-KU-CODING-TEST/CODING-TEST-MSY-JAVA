-- !!! 예시 화면의 빈칸은 NULL을 의미하는 시각적 표현이며,
-- !!! 별도의 변환 요구가 없다면 원본 컬럼의 NULL 상태와 날짜 타입을 그대로 유지하는 것이 SQL 코딩테스트의 정석 규칙

-- !!! 프로그래머스의 대다수 날짜 관련 문제(예: 대여 기록, 입양 시각 구하기 등)는 예시 화면에는 날짜만 나와 있어도(예: 2022-10-05),
-- !!! 지문 상세 내용에 "결과는 DATE 포맷이 %Y-%m-%d 형태여야 합니다"라는 문구가 숨어 있는 경우가 많음, 이 문구가 있다면 반드시 DATE_FORMAT 함수를 써야 정답 처리가 됨
-- !!! DATE_FORMAT(컬럼명, '%Y-%m-%d'), 소문자 %y를 쓰면 연도가 2자리(26)로 나오니 반드시 문제 요구사항에 맞춰 대소문자를 확인
--- 1. 연·월·일 (Date) 규칙 : 대소문자에 따라 글자 수가 달라지거나 월 이름(영어)으로 표시됩니다.
----- %Y (대문자) : 2026 | 연도 4자리 (가장 많이 씀)
----- %y (소문자) : 26 | 연도 뒤의 2자리
----- %m (소문자) : 09 | 월 2자리 (01~12) (가장 많이 씀)
----- %M (대문자) : September | 월의 영어 이름 전체
----- %b (소문자) : Sep | 월의 영어 이름 축약형
----- %c (소문자) : 9 | 월 1~2자리 (앞에 0 안 붙음)
----- %d (소문자) : 05 | 일 2자리 (01~31) (가장 많이 씀)
----- %e (소문자) : 5 | 일 1~2자리 (앞에 0 안 붙음)

--- 2. 시·분·초 (Time) 규칙 : 24시간제와 12시간제 구분에 주의해야 합니다.
----- %H (대문자) : 14 | 24시간제 시간 2자리 (00~23) (가장 많이 씀)
----- %k (소문자) : 14 | 24시간제 시간 1~2자리 (앞에 0 안 붙음)
----- %h (소문자) / %I : 02 | 12시간제 시간 2자리 (01~12)
----- %l (소문자 L) : 2 | 12시간제 시간 1~2자리 (앞에 0 안 붙음)
----- %i (소문자) : 07 | 분 2자리 (00~59) (가장 많이 씀)
----- %s (소문자) / %S : 03 | 초 2자리 (00~59) (가장 많이 씀)
----- %p (소문자) : PM | 오전/오후 표시 (AM 또는 PM)
